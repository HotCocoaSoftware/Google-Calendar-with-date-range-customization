...
public class Utils {
    ...
    public static boolean checkValidDay(SimpleMonthAdapter.CalendarDay calendarDay, DatePickerController datePickerController) {
        Calendar minDate = datePickerController.getMinDate();
        Calendar maxDate = datePickerController.getMaxDate();
        if (minDate != null && maxDate != null && (calendarDay.getCalendar().compareTo(minDate) != 1 || (calendarDay.getCalendar()).compareTo(maxDate) == 1)) {
            return false;
        } else {
            return true;
        }
